...
public class PatientController {
	...
	public PatientResponse patientLogin(String email, String password) {
		PatientInfoBean patient = patientService.loginPatient(email, password);
		PatientResponse response = new PatientResponse();
		if (patient != null) {
			response.setStatusCode(200);
			response.setError(false);
			response.setMessage("patient profile found");
			response.setBean(patient);
		}
		return response;
	}
	...
	@GetMapping(path = "/getpatientprofile", produces = MediaType.APPLICATION_JSON_VALUE)
	public PatientResponse getPatientProfile(int patientId) {
		PatientInfoBean patient = patientService.getPatientProfile(patientId);
		PatientResponse response = new PatientResponse();
		if (patient != null) {
			response.setStatusCode(200);
			response.setError(false);
	...
	@DeleteMapping(path = "/deleteappointmentbypatient", produces = MediaType.APPLICATION_JSON_VALUE)
	...
	public FeedBackResponse addPatientFeedBack( @RequestBody FeedBackInfoBean bean) {
	...
	public AppointmentResponse getAllAppointmentsByPatient(int patientId) {
		...
		if (list != null) {
			response.setStatusCode(200);
			response.setError(false);
			response.setMessage("Appointments details found");
	...
	public FeedBackResponse getFeedBacksOfDoctor( int doctorId) {
		...
		if (feedBacks != null) {
			response.setStatusCode(200);
			response.setError(false);
			response.setMessage("FeedBack on Doctor");
